...
export default function HomePage() {
  ...
  const [loadingText, setLoadingText] = useState('Fetching real time data...');

  useEffect(() => {
    // Only show loading if authenticated (so we don't block the landing page)
    if (!isAuthenticated) {
    ...
    const startLoadingSequence = () => {
      ...
      // Fake 7 second delay
      timer1 = setTimeout(() => {
        setLoadingText('Generating insights and maps...');
      }, 3500);
      ...
      <AnimatePresence>
        {isInitialLoading && isAuthenticated && (
          ...
          >
            ...
            >
              <div className="absolute inset-0 rounded-[28px] bg-gradient-to-br from-blue-500/10 to-purple-500/10" />
            ...
            <AnimatePresence mode="wait">
              <motion.h3 
                ...
                exit={{ opacity: 0, y: -10 }}
                className="text-[13px] font-bold tracking-[0.3em] uppercase text-white/90 drop-shadow-lg animate-pulse"
              >
                {loadingText}
              </motion.h3>
            </AnimatePresence>

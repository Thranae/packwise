...
export function Sidebar() {
  ...
  return (
    ...
    >
      ...
      {/* Navigation */}
      <nav className="flex-1 flex flex-col justify-between overflow-y-auto px-3 py-3">
        {navItems.map((item) => {
          ...
          return (
            <NavLink
              key={item.path}
              to={item.path}
              className={`relative flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm group overflow-hidden transition-all duration-700 ease-[cubic-bezier(0.16, 1, 0.3, 1)] ${!isActive ? 'hover:bg-gradient-to-br hover:from-white/[0.12] hover:to-white/[0.02] hover:shadow-[inset_0_1px_1px_rgba(255,255,255,0.4),0_8px_16px_rgba(0,0,0,0.2)] hover:-translate-y-0.5 hover:scale-[1.02]' : ''}`}
            >

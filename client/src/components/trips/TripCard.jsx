...
export const TripCard = ({ trip }) => {
  ...
  return (
    ...
    >
      ...
      >
        {/* Glow Effect for mobile (static soft glow instead of hover-based) */}
        {!isNative && (
          <div
            className="absolute -inset-[1px] rounded-[32px] opacity-100 transition-opacity duration-500 -z-10 blur-xl will-change-opacity transform-gpu"
            style={{ background: `linear-gradient(to bottom right, ${glowColor}15, transparent)`, transform: 'translateZ(0)' }}
          />
        )}
        ...
        <div className="p-6 flex flex-col justify-between flex-1 relative ios-3d-element bg-transparent">
          ...
          <div className="mt-5 relative z-20">
            {isEditing ? (
              ...
              <button onClick={(e) => { e.stopPropagation(); selectTrip(trip._id); triggerTransition(ROUTES.OVERVIEW, { text: 'Generating insights & maps...' }); }} className="w-full flex items-center justify-center py-3.5 rounded-[16px] ios-liquid-button text-white font-bold text-sm tracking-wide transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] active:scale-95 cursor-pointer group/btn">
                Open Trip Overview
              </button>
            )}
          </div>
